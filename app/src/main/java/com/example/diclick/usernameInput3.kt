...
class usernameInput3 : AppCompatActivity() {

    private lateinit var mUserViewModel: UserViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_username_input3)
        ...
        enter.setOnClickListener(){
            ...
            if(!userData.isEmpty()){
                var intent = Intent(this@usernameInput3, testHandLevel3::class.java)
                intent.putExtra("userintent",userData)
                startActivity(intent)
            }
        }
        delete.setOnClickListener(){
            mUserViewModel.deleteAllUsers3()
