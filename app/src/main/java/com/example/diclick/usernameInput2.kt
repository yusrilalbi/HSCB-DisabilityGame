...
class usernameInput2 : AppCompatActivity() {

    private lateinit var mUserViewModel: UserViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_username_input2)
        ...
        enter.setOnClickListener(){
            ...
            if(!userData.isEmpty()){
                var intent = Intent(this@usernameInput2, testHandLevel2::class.java)
                intent.putExtra("userintent",userData)
                startActivity(intent)
            }
        }
        delete.setOnClickListener(){
            mUserViewModel.deleteAllUsers2()
